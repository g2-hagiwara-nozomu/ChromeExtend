class PageAction_Jenkins {
  ...
  static OnExtend(url) {
    ...
    chrome.storage.local.get(["enableJenkins"], function (items) {
      let enabled = items.enableJenkins;
  ...
  static DoExtend(url) {
    ...
    let position = document.body.clientWidth / 2 - ((PageAction_Jenkins.width * jobDetailList.length) / 2.0) - 15;
  ...
  static DiableExtend() {
    ...
    // 向こうを保存
    chrome.storage.local.set({ "enableJenkins": false });
  }
}